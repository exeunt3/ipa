...
Prose is primary. Taxonomies are optional lenses for recall and comparability.
</p>

<section style={card}>
<label style={{ display: "block", fontWeight: 600, marginBottom: 8 }}>Protocol</label>
<select
value={protocolSlug}
...
<div style={{ opacity: 0.8, fontSize: 13 }}>Coarse sliders for comparison.</div>
</div>
<label style={{ display: "flex", gap: 8, alignItems: "center" }}>
<span style={{ fontSize: 13, opacity: 0.8 }}>Attribution</span>
<select value={anonymity} onChange={(e) => setAnonymity(e.target.value as any)} style={input}>
...
<div style={{ opacity: 0.8, fontSize: 13 }}>Reference vocabulary only. Select up to {seiCap}.</div>
</div>
<a href="https://www.effectindex.com/effects" target="_blank" rel="noreferrer" style={{ fontSize: 13 }}>
...
{seiEffects.length === 0 ? <span style={{ opacity: 0.65, fontSize: 13 }}>No SEI effects selected.</span> : null}
</div>
...
background: "white",
cursor: "pointer",
opacity: seiEffects.includes(s.id) ? 0.5 : 1,
}}
>
...
const card: React.CSSProperties = { marginTop: 16, padding: 16, border: "1px solid rgba(0,0,0,0.12)", borderRadius: 12, background: "white" };
...
<span style={{ opacity: 0.75, fontSize: 13 }}>{props.value}</span>
</div>
...
return (
<span style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "4px 10px", borderRadius: 999, border: "1px solid rgba(0,0,0,0.14)", fontSize: 12 }}>
{text}
<button type="button" onClick={onRemove} style={{ border: "none", background: "transparent", cursor: "pointer", fontSize: 14, lineHeight: 1, opacity: 0.7 }}>
×
</button>
</span>
);
...
<span style={{ fontWeight: 600, fontSize: 13, opacity: 0.85 }}>{label}</span>
